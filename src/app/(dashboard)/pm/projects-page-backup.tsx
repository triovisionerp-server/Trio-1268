...
export default function ProjectManagerDashboard() {
  ...
  useEffect(() => {
    setIsLoading(true);
    const projectsRef = collection(db, 'pm_projects');
  ...
  // DATA OPERATIONS
  // ==========================================
  const handleCellChange = async (index: number, field: string, value: string | number) => {
    ...
    if (field === 'totalParts' || field === 'totalPartsProduced') {
      const total = field === 'totalParts' ? parseFloat(String(value)) : (project.totalParts || 0);
      const produced = field === 'totalPartsProduced' ? parseFloat(String(value)) : (project.totalPartsProduced || 0);
      const percent = total > 0 ? Math.round((produced / total) * 100) : 0;
      
      updates.percentCompleted = percent;
      updates.totalPartsToBeProduced = Math.max(0, total - produced);
      
      if (percent >= 100) updates.status = 'Completed';
      else if (percent > 0) updates.status = 'In Progress';
      else if (!project.status) updates.status = 'Pending';
    }

    try {
      setIsSyncing(true);
      const projectRef = doc(db, 'pm_projects', String(project.id));
  ...
  const addNewRow = async () => {
    ...
    try {
      setIsSyncing(true);
      await addDoc(collection(db, 'pm_projects'), newProject);
      toast.success('New project row added');
    } catch (error) {
  ...
  const deleteProject = async (id: string | number) => {
    ...
    try {
      setIsSyncing(true);
      await deleteDoc(doc(db, 'pm_projects', String(id)));
  ...
  const updateProject = async (id: string | number, updates: Partial<Project>) => {
    try {
      ...
      if (updates.totalParts !== undefined || updates.totalPartsProduced !== undefined) {
        const total = updates.totalParts ?? project.totalParts;
        const produced = updates.totalPartsProduced ?? project.totalPartsProduced;
        updateData.totalPartsToBeProduced = Math.max(0, total - produced);
        updateData.percentCompleted = total > 0 ? Math.round((produced / total) * 100) : 0;
        if (updateData.percentCompleted >= 100) updateData.status = 'Completed';
      }
      
      const projectRef = doc(db, 'pm_projects', String(id));
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const mapped = json.map((row: any) => ({
          ...
          remarks: row['Remarks'] || '',
          containerNumber: row['Container #'] || '',
          dispatchDate: row['Dispatch'] || '',
          createdAt: Timestamp.now().toMillis()
        }));
        ...
        const promises = processed.map(project => 
          addDoc(collection(db, 'pm_projects'), project)
        );
        await Promise.all(promises);
        
        toast.success(`Imported ${processed.length} projects successfully`);
      } catch (e) {
  ...
  const clearAll = async () => {
    ...
    try {
      setIsSyncing(true);
      const snapshot = await getDocs(collection(db, 'pm_projects'));
    ...
    <div className="min-h-screen bg-zinc-950 text-white font-sans pb-20 relative overflow-hidden">
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-zinc-900 border border-white/10 rounded-2xl p-8 flex flex-col items-center gap-4">
            <RefreshCw className="w-12 h-12 text-purple-500 animate-spin" />
            <p className="text-white font-semibold">Loading Projects...</p>
            <p className="text-zinc-400 text-sm">Syncing with Firebase</p>
          </div>
        </div>
      )}
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <button
              ...
              disabled={isSyncing}
              className="flex items-center gap-2 px-4 py-2.5 bg-zinc-800 text-white rounded-xl hover:bg-zinc-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              disabled={isSyncing}
              className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-purple-500 to-pink-600 text-white font-semibold rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <div className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm ${
              ...
              <Activity className={`w-4 h-4 ${isSyncing ? 'animate-spin' : 'animate-pulse'}`} />
            ...
            <div className="p-4 border-b border-white/10 flex flex-wrap justify-between items-center gap-4 bg-white/5">
              <div className="flex gap-2 flex-wrap">
                <button 
                  onClick={addNewRow}
                  disabled={isSyncing}
                  className="bg-purple-600 hover:bg-purple-500 text-white px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button 
                  ...
                  disabled={isSyncing}
                  className="bg-green-600 hover:bg-green-500 text-white px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button 
                  ...
                  className="bg-zinc-700 hover:bg-zinc-600 text-white px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button 
                  onClick={clearAll}
                  disabled={isSyncing || data.length === 0}
                  className="bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
