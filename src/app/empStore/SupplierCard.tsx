...
interface SupplierCardProps {
  supplier: DBSupplier;
  onViewDetails: (supplier: DBSupplier) => void;
}

export function SupplierCard({ supplier, onViewDetails }: SupplierCardProps) {
  const relationshipDuration = () => {
    const years = differenceInYears(new Date(), supplier.relationship_start || new Date());
    const months = differenceInMonths(new Date(), supplier.relationship_start || new Date()) % 12;
